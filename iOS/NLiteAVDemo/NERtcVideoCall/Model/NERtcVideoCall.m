...
- (void)call:(NEUser *)user completion:(void (^)(NSError *__nullable error))completion {
  NERtcVideoCallModel *model =
      [[NERtcVideoCallModel alloc] initWithLocalUser:self.localUser
                                          remoteUser:user
  ...
  request.customInfo = [NSString stringWithFormat:@"%@,%@", self.currentModel.localUser.mobile,
                                                  self.currentModel.localUser.avatar];
  ...
  info.pushTitle = @"网易云信";
  info.pushContent =
      [NSString stringWithFormat:@"%@邀请你视频通话", self.currentModel.localUser.mobile];
  ...
  [[[NIMSDK sharedSDK] signalManager]
      signalingCall:request
         completion:^(NSError *_Nullable error,
                      NIMSignalingChannelDetailedInfo *_Nullable response) {
           ...
           [[NERtcEngine sharedEngine]
               joinChannelWithToken:token
                        channelName:self.currentModel.channelID
                              myUid:[self.currentModel.localUser.imAccid longLongValue]
                         completion:^(NSError *_Nullable error, uint64_t channelId,
                                      uint64_t elapesd) {
      ...
      signalingCancelInvite:request
                 completion:^(NSError *_Nullable error) {
                   ...
                   if (!error) {
                     ...
                     NIMSignalingCloseChannelRequest *closeRequest =
                         [[NIMSignalingCloseChannelRequest alloc] init];
                     closeRequest.channelId = self.currentModel.channelID;
                     [[[NIMSDK sharedSDK] signalManager]
                         signalingCloseChannel:closeRequest
                                    completion:^(NSError *_Nullable error){
                                    }];
                   }
                 }];
...
- (void)reject:(void (^)(NSError *__nullable error))completion {
  ...
  [[[NIMSDK sharedSDK] signalManager] signalingReject:rejectRequest
                                           completion:^(NSError *_Nullable error) {
                                             [[NERtcEngine sharedEngine] leaveChannel];
                                             if (completion) {
                                               completion(error);
...
- (void)hangup {
  ...
  request.channelId = self.currentModel.channelID;
  [[[NIMSDK sharedSDK] signalManager] signalingLeaveChannel:request
                                                 completion:^(NSError *_Nullable error){
                                                 }];
  NIMSignalingCloseChannelRequest *closeRequest = [[NIMSignalingCloseChannelRequest alloc] init];
  closeRequest.channelId = self.currentModel.channelID;
  [[[NIMSDK sharedSDK] signalManager] signalingCloseChannel:closeRequest
                                                 completion:^(NSError *_Nullable error){
                                                 }];
  [[NERtcEngine sharedEngine] leaveChannel];
}
  ...
  NERtcVideoCallModel *model =
      [[NERtcVideoCallModel alloc] initWithLocalUser:self.localUser
                                          remoteUser:remoteUser
                                           ...
                                           channelId:info.channelInfo.channelId];
  self.currentModel = model;
...
- (void)nimSignalingOnlineNotifyEventType:(NIMSignalingEventType)eventType
  ...
  switch (eventType) {
    ...
    case NIMSignalingEventTypeCancelInvite: {
      self.callStatus = NECallStatusNone;
      NIMSignalingCancelInviteNotifyInfo *info =
          (NIMSignalingCancelInviteNotifyInfo *)notifyResponse;
      for (id<NERtcVideoCallDelegate> delegate in self.delegateList) {
    ...
    case NIMSignalingEventTypeReject: {
      ...
      [[NIMSDK sharedSDK].signalManager signalingCloseChannel:request
                                                   completion:^(NSError *_Nullable error){
                                                   }];
      [[NERtcEngine sharedEngine] leaveChannel];
...
- (void)nimSignalingOfflineNotify:(NSArray<NIMSignalingNotifyInfo *> *)notifyResponse {
  ...
  [[[NIMSDK sharedSDK] signalManager]
      signalingQueryChannelInfo:request
                     completion:^(NSError *_Nullable error,
                                  NIMSignalingChannelDetailedInfo *_Nullable response) {
                       if (!error) {
...
- (NSString *)generateRequestID {
  ...
  NSString *requestID =
      [NSString stringWithFormat:@"%.f%zd", [NSDate timeIntervalSinceReferenceDate], random];
  return requestID;
}
