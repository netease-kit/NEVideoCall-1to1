...
- (instancetype)init {
  ...
  if (self) {
    ...
    _highlightBorderColor = [UIColor colorWithRed:255 / 255.0
                                            green:70 / 255.0
                                             blue:62 / 255.0
                                            alpha:1];
    _cornerRadius = 4;
...
- (void)setupSubviews {
  ...
  CGFloat padding =
      (CGRectGetWidth(self.frame) - _maxLenght * CGRectGetHeight(self.frame)) / (_maxLenght - 1);
  UIView *lastView;
  for (int i = 0; i < self.maxLenght; i++) {
    ...
    subView.frame = CGRectMake(leftOffset, 0, CGRectGetHeight(_contairView.frame),
                               CGRectGetHeight(_contairView.frame));
    ...
    CGRect rect = CGRectMake((CGRectGetHeight(self.frame) - 2) / 2.0, 5, 2,
