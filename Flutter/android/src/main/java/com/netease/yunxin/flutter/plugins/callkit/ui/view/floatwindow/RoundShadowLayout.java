...
public final class RoundShadowLayout extends FrameLayout {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    for (int i = 0; i < getChildCount(); ++i) {
      ...
      int childWidthSpec =
          FrameLayout.getChildMeasureSpec(widthMeasureSpec - (int) mShadowRadius * 2, 0, lp.width);
      int childHeightSpec =
          FrameLayout.getChildMeasureSpec(
              heightMeasureSpec - (int) mShadowRadius * 2, 0, lp.height);
    ...
    setMeasuredDimension(
        width + getPaddingLeft() + getPaddingRight() + (int) mShadowRadius * 2,
  ...
  private void setBackgroundCompat(int width, int height) {
    Bitmap bitmap =
        createShadowBitmap(width, height, mShadowRadius, mShadowX, mShadowY, mShadowColor);
    BitmapDrawable drawable = new BitmapDrawable(getResources(), bitmap);
    setBackground(drawable);
  }

  private Bitmap createShadowBitmap(
      int shadowWidth, int shadowHeight, float shadowRadius, float dx, float dy, int shadowColor) {
    Bitmap var10000 = Bitmap.createBitmap(shadowWidth, shadowHeight, Config.ARGB_8888);
    Intrinsics.checkNotNullExpressionValue(
        var10000, "Bitmap.createBitmap(shad… Bitmap.Config.ARGB_8888)");
